...
export async function createRepositorySubmission(
  ...
  if (existing) {
    throw new Error("This scholarly work already exists in the repository queue.");
  }
...
export async function getPublishedResearcher(slug: string) {
  ...
  const works = await RepositoryWork.find({
    researcherId: researcher._id,
    status: "APPROVED",
  })
...
export async function getPublishedPdf(
  slug: string,
): Promise<RepositoryPdfRecord | null> {
  await connectMongoDB();

  return await RepositoryWork.findOne({
    slug,
    status: "APPROVED",
...
export async function listPendingRepositoryWorks(limit = 100) {
  ...
  const works = await RepositoryWork.find({ status: "SUBMITTED" })
    ...
    .populate(
      "researcherId",
      "+contactEmail slug fullName affiliation department orcid",
    )
    .lean();
...
export async function moderateRepositoryWork(input: {
  ...
  if (input.action === "approve") {
    if (!input.rightsReviewed || !input.metadataReviewed) {
      throw new Error(
        "Rights and metadata must both be reviewed before approval.",
      );
    }
...
export async function getPendingRepositoryPdf(
  id: string,
): Promise<RepositoryPdfRecord | null> {
  await connectMongoDB();
...
export async function getRepositorySitemapEntries() {
  ...
  try {
    ...
    const [works, researchers] = await Promise.all([
      RepositoryWork.find({ status: "APPROVED" })
